...
set, and write <root>_reweighted.scnf with positive-integer weights.  Returns
the output pathname.  Prints a target-vs-achieved marginal report when VERBOSE."
  ...
  (multiple-value-bind (clauses probabilities options) (rw--read-scnf scnf-file)
    (let ((soft '()) (degenerate '()))           ; soft: (atom . p);  degenerate: (atom . 0/1)
      (dolist (tg (rw--collect-targets probabilities))
        (let ((p (cdr tg)))
          (cond ((= p 0d0) (push (cons (car tg) 0) degenerate))
                ((= p 1d0) (push (cons (car tg) 1) degenerate))
                (t (push (cons (car tg) (float p 1d0)) soft)))))
      (setf soft (nreverse soft) degenerate (nreverse degenerate))
      (let* ((deg-clauses (mapcar (lambda (d)
                                    (if (= (cdr d) 1)
                                        (list 'or (car d))
                                        (list 'or (list 'not (car d)))))
                                  degenerate))
             (all-clauses (append clauses deg-clauses))
             (soft-atoms (mapcar #'car soft)))
        (multiple-value-bind (a2i nvars) (mx--index-atoms all-clauses soft-atoms)
          (let* ((int-clauses (mapcar (lambda (cl) (mx--clause->ints cl a2i)) all-clauses))
            ...
            (multiple-value-bind (thetas marginals iters converged)
                (mx--fit entries (mapcar #'cdr soft) :eta eta :tol tol :max-iters max-iters)
              (unless out-file
                (setq out-file (cl-ppcre:regex-replace "\\.[^.]*$" scnf-file "_reweighted.scnf")))
              (let ((new-weights '()))
                (loop for atom in soft-atoms for a from 0
                      for wf = (rw--emit-for-theta atom (aref thetas a) scale)
                ...
                (with-open-file (out out-file :direction :output
                  ...
                  (format out "; method: exact iterative MaxEnt over the feasible set; scale: ~D~%" scale)
                  ...
                  (loop for atom in soft-atoms for a from 0 do
                    (format out "; marginal ~S target ~,4F achieved ~,4F~%"
                            atom (cdr (nth a soft)) (aref marginals a)))
                ...
                (when verbose
                  (format t "~&MaxEnt reweight: ~A (~D iters)~%"
                          (if converged "converged" "did NOT converge") iters)
                  (format t "~&  atom~30Ttarget~42Tachieved~%")
                  (loop for atom in soft-atoms for a from 0 do
                    (format t "~&  ~S~30T~,4F~42T~,4F~%"
                            atom (cdr (nth a soft)) (aref marginals a)))
                  (when degenerate
                    (format t "~&  (hard) ~{~S ~}~%" (mapcar #'car degenerate)))
                  (format t "~&  -> ~A~%" out-file))
                out-file))))))))
