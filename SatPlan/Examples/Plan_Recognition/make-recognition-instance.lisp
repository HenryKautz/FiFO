...
(defun add-requirements (reqs)
  "Extend a (:requirements ...) body with :action-costs and
:disjunctive-preconditions (the generated problem's goal is a disjunction)."
  (let ((body (copy-list reqs)))
    (dolist (r '(:action-costs :disjunctive-preconditions) body)
...
(defun transform-domain (domain-def cost)
  "DOMAIN-DEF with :action-costs machinery and uniform action costs."
  (let ((has-functions nil) (sections '()))
    ...
    (dolist (s (cddr domain-def))
      (cond ((and (consp s) (sym= (first s) "REQUIREMENTS"))
            ...
            (t (push s sections))))
    (list* (first domain-def) (second domain-def) (nreverse sections))))
...
HYPS (each a list of goal atoms) and (= (total-cost) 0) added to :init."
  (let ((goal `(or ,@(mapcar (lambda (h) `(and ,@h)) hyps))))
      ...
      (write-forms domain-out
                   (list (format nil "Generated by make-recognition-instance.lisp from ~a:"
                         ...
                         (format nil "multiplies a trajectory's probability by exp(-~a/scale))."
                                 cost))
                   (list (transform-domain domain-def cost)))
