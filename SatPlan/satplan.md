...
| `(occur-in-order [M [N]] a₁ … aₖ)` | the ground actions occur **in this order**, at strictly increasing slices within `[M, N]` | a monitor chain — see below |
| `(occur-in-nonstrict-order [M [N]] a₁ … aₖ)` | the same, but at **non-decreasing** slices, so two may share one | the same chain, plus a within-slice one |

`occur-in-order` takes an optional slice window before the actions, and since an action is always a list while a bound is an integer, the three forms never collide:

```lisp
(occur-in-order a₁ … aₖ)        ; anywhere in the plan   (= 1 -1)
(occur-in-order M a₁ … aₖ)      ; not before slice M     (= M -1)
(occur-in-order M N a₁ … aₖ)    ; within [M, N];  N = -1 means no upper bound
```

It asserts an *order-preserving embedding*: strictly increasing slices `t₁ < … < tₖ` inside the window, with `aᵢ` occurring at `tᵢ` — anything else may happen in between, and the observations need not be consecutive. The compilation is the `ObsDone` monitor described under [Conditioning on evidence and marginal inference](#conditioning-on-evidence-and-marginal-inference), which is where the operator started life as an evidence form; the window is a quantifier **guard** on the progression axioms, with a second, complementary quantifier freezing the chain outside it. That is deliberate rather than incidental. Naming slices `M` and `N+1` directly would emit a slice constant that may not exist at the current horizon, and an `ObsDone` atom no other clause mentions makes the whole constraint quietly vacuous — the failure this encoding is shaped to avoid. Guarding cannot: a window too short for `k` observations, or one wholly past the horizon, simply leaves `ObsDone(k, numslices)` unreachable and the theory unsatisfiable, the same way `occur-sometime`'s empty `exists` fails. The unwindowed form is exactly the `1 -1` case, with the outside quantifier empty.

`occur-in-nonstrict-order` differs in one respect: consecutive observations may land on the **same** slice. That matters because FiFO's slices are *parallel*. Strict ordering forces simultaneous observations apart, asserting an order the plan never claimed and charging for the serialization — on a small logistics plan whose two loads happen together at slice 1, `occur-in-order` costs 17 where the unconstrained plan costs 16, and `occur-in-nonstrict-order` costs 16 with both loads left at slice 1. On the IntrusionDetection benchmark, whose plans act on several hosts at once, six such observations are outright *unsatisfiable* under strict ordering and reproduce the source plan under non-strict. If your observer could not distinguish simultaneous actions, non-strict is the honest reading.

It adds a second monitor family, `ObsAt(c, i, s)` — "the first *i* are explained by actions up to **and including** *s*" — recursing on *i* within a slice, with `ObsDone(i, s+1) ⟺ ObsAt(i, s)` carrying the result forward. Same O(k·numslices), same determinism, so still count-neutral.

One asymmetry is deliberate: two **adjacent identical** observations still may not share a slice. `Occurs` is boolean per (action, slice) — there is no "twice at *s*" — so allowing it would let one occurrence satisfy both, silently breaking the guarantee that each observation binds a distinct occurrence. That pair alone falls back to the strict rule.

Like the evidence form, both take **ground** actions only, so neither can appear under a `forall`, and each action is validated against the domain. `(not (occur-in-order …))` works here too, windows included, and the same three arities and both operators are accepted by `--pddl-evidence`.
...
```lisp
(:goal (and
      (or (at pkg1 c1-p1) (at pkg1 c2-p2) (at pkg1 c1-p3))       ; hard: pkg1 reaches one
      (or (at pkg2 c1-p2) (at pkg2 c2-p2) (at pkg2 c2-p3))       ; hard: pkg2 reaches one
      (preference deliver-pkg1-1 (at pkg1 c1-p1) 9)              ; soft, graded
      (preference deliver-pkg1-2 (at pkg1 c2-p2) 5)
      (preference deliver-pkg1-3 (at pkg1 c1-p3) 4)
      ...))
```
...
**Exporting the observations as an ordering constraint.** `--export-ordering-constraints <dir>` is the middle ground between the two above: it writes **one** constraint carrying the observations' order and the span of the plan they fell in, without pinning any of them to a slice.

```lisp
(:constraints (and (occur-in-order 3 4 (load pkg1 truck1 c1-p2) (fly plane1 c2-air c1-air))))
```

Same three files as `--export-constraints`. `occur-in-order` is a FiFO extension with no standard counterpart at all — PDDL 3.0 cannot say an action occurred, let alone that several occurred in order — so unlike `hold-during` there is no standard fallback, and the caveat comment is **unconditional** rather than emitted only when actions appear.

**Two observations in the same slice are refused** — unless you ask for the operator that can express them. `occur-in-order` requires strictly increasing slices while FiFO's are *parallel*, so writing two simultaneous actions as a sequence would demand they occupy different slices, which the source plan never claimed, and the exported problem could come back unsatisfiable. So the strict export names the offending slice and stops.

`--nonstrict-ordering 1` emits `(occur-in-nonstrict-order …)` instead and accepts the tie. That is the usual answer on a parallel domain: six IntrusionDetection observations spanning slices 1–3 export cleanly this way and solve at the source plan's cost, where the strict form of the same six is unsatisfiable. The alternatives are to narrow with `--slices` (drop the crowded ones) or `--observe` — noting it filters by predicate *name*, so it cannot separate two groundings of the same action — or to use `--export-constraints`, which pins each observation to its own slice.
