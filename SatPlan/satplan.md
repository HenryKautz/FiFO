...
Like the evidence form, it takes **ground** actions only, so it cannot appear under a `forall`, and each is validated against the domain. `(not (occur-in-order …))` works here too, windows included, and the same three forms are accepted by `--pddl-evidence`.
...
| **Ordering constraint** | `--export-ordering-constraints <dir>` | a planner, directly, when only the **order** is claimed. One `(occur-in-order M N …)` spanning the observed slices; actions only, and two observed in the same slice are refused |
...
**Two observations in the same slice are refused.** `occur-in-order` requires strictly increasing slices, and FiFO's slices are *parallel*: writing two simultaneous actions as a sequence would demand they occupy different slices, which the source plan never claimed, and the exported problem could come back unsatisfiable. So the export names the offending slice and stops. Narrow with `--slices` (drop the crowded ones) or `--observe` — noting it filters by predicate *name*, so it cannot separate two groundings of the same action — or use `--export-constraints`, which pins each observation to its own slice and handles parallel ones natively.
