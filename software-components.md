...
| `tests/run-test-ppgen.sh` | `SatPlan/ppgen.sh`, the clara-logistics problem generator: structural checks against the spec (clique/grid topology, even distribution, maximal airport separation verified by brute force, complete routes, goals that differ from starts), the preference mode (disjunctive goal with equally-spaced weights), `--maxgoals` capping the deliveries, the recorded settings block and seed round-trip, eleven error paths, and a solvability check per style. Skips the solvability cases without a MaxSAT solver. |
