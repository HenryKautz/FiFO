...
**One instantiation, not 2n.** The problem is instantiated **once** at the
horizon and the `2n` costs come from clamping `hypI` on that single scnf, through
[`marginals.sh --hypotheses`](#marginalssh). Clamping `hypI` in the disjunctive
theory selects the same models as rewriting the goal to `Gᵢ`, since `hypI` implies
the disjunction (`T∨ ∧ hypI ≡ Tᵢ`) — so it is the same computation the older
implementation did with `2n` translate-instantiate-solve cycles through
`planner.sh`. Measured on IntrusionDetectionCosts at horizon 6, same exact MaxSAT solver on
both sides: **4.1 s against 28.6 s**, `summary.tsv` byte-identical. The old implementation is kept as
`--method plan-runs`, and the test suite checks the two against each other.

`(occur-in-order …)` reaches that single scnf through a **split**. Its translation
is `(and <monitor axioms> <assertion>)`; the axioms are biconditional, so every
`ObsDone` atom is determined by the action trace and adding them to the theory is
count-neutral, while the assertion is the one literal that claims anything about
what was observed. `planner.sh --split-evidence` folds the axioms in and writes
the assertion — **resolved**, since the source form says `numslices`, which is
bound only in the instantiation environment — to `<root>-assertion.txt`.
Conditioning on that literal gives the comply case and negating it the
does-not-comply case, both from one instantiation.
...
| `--method <m>` | `fast` (default) or `plan-runs`, the older 2n-planner-run implementation, kept as the reference the fast path is tested against. It always computes R&G's difference, so `--counter` and `--baseline` are *refused* with it rather than accepted and ignored. |
| `--maxsat-solver <name>` | The solver producing the **costs**, on both paths. Default `bin/rc2-maxsat.py`, which is **exact** — and checked to be *usable*, not merely executable, since the bundled script runs without `python-sat` installed and would then yield no costs at all. R&G's score is a *difference of two minima*, so an anytime solver yields two upper bounds that do not cancel — the same hazard `max-term` warns about. The plan-runs path silently had it, since `planner.sh`'s default weighted solver is anytime; that made repeated runs disagree, and was slower here (120 s vs 28.6 s for identical work). |
...
`(not A B)` is not a formula. A multi-form file is refused, naming
`evgen.sh --recognition 1` as the fix.

**Slice-pinned (`fifo`) evidence is horizon-dependent**, unlike the modal forms.
Since both runs happen at one fixed horizon, the horizon is raised to at least the
largest slice the evidence names — below that no plan can satisfy it, every
`c(O)` would be infinite, and every posterior 0. A fluent observed at the *final*
slice means "at the end" only at the horizon it came from, so observations
restricted to action names are the portable choice.
...
approximation ([probability-background.md §3.2](Probability/probability-background.md#32-maximum-term-approximation-of-the-partition-function))
per atom:
...
| `tests/run-test-pddl.sh` | PDDL-translator regression: for every example with a checked-in translation under `SatPlan/Examples/`, runs `pddl2fifo` on a temp copy and diffs the `.wff` byte-for-byte (the `(include ...)` satplan path is normalized), plus one behavioral case that resolves that include path through a symlinked problem directory. |
| `tests/run-test-evidence.sh` | `--pddl-evidence`, chiefly `occur-in-order`: behavioral checks (the plan embeds the observed sequence at strictly increasing slices; bad evidence raises its contextual error), not gold diffs — evidence clauses can contain session-varying gensyms.. Also covers the evidence/theory check: a misspelled predicate, fluent or derived predicate is an error naming the atom, a real atom at a slice past the horizon is UNSAT rather than an error (so the horizon search extends to one that fits it), a negated out-of-range literal is vacuously true, and derived-predicate evidence still works. Also the windowed `occur-in-order` constraint operator: the three arities agreeing on an unwindowed plan, a window that binds, one too short and one past the horizon both UNSAT rather than vacuous, negation, four bound errors, and the same arities working as evidence, plus `occur-in-nonstrict-order`: parallel observations kept in one slice where strict costs +1, and two identical observations still needing two occurrences — the adjacent-duplicate rule, which a naive non-strict encoding gets silently wrong |
