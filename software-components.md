...
| `tests/run-test-evidence.sh` | `--pddl-evidence`, chiefly `occur-in-order`: behavioral checks (the plan embeds the observed sequence at strictly increasing slices; bad evidence raises its contextual error), not gold diffs — evidence clauses can contain session-varying gensyms.. Also covers the evidence/theory check: a misspelled predicate, fluent or derived predicate is an error naming the atom, a real atom at a slice past the horizon is UNSAT rather than an error (so the horizon search extends to one that fits it), a negated out-of-range literal is vacuously true, and derived-predicate evidence still works |
